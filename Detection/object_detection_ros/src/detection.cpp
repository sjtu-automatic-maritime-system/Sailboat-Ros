...
//    cv::Mat img_clone = img_src.clone();
    ...
    cv::Mat output = hls_channels[2].mul(hls_channels[1]/255);
    //imshow("output", output);
    cv::waitKey(5);

    cv::GaussianBlur(output, edge, cv::Size(9, 9), 2, 2);
...
std::vector<cv::Vec3f> circleDectection(cv::Mat &src_img, cv::Mat edge) {
    ...
    cv::HoughCircles(edge, circles, CV_HOUGH_GRADIENT, 2, 100, 200, 100, 0, 0);
