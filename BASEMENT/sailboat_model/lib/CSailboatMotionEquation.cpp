...
void CSailboatMotionEquation::KeelCoefInit() {
    //lookup table
    double xdata[37];
    //*0.8
...
void CSailboatMotionEquation::HullCoefInit() {
    //lookup table
    double xdata[37];
    //0.3*
...
void CSailboatMotionEquation::RudderCoefInit() {
    //lookup table
    double xdata[37];
...
void CSailboatMotionEquation::SailCoefInit() {
    //lookup table
...
double* CSailboatMotionEquation::KeelCoef(double alpha) {
...
double* CSailboatMotionEquation::HullCoef(double alpha) {
...
double* CSailboatMotionEquation::RudderCoef(double alpha) {
...
double* CSailboatMotionEquation::SailCoef(double alpha) {
    //generate a lookup table for the lift/drag coefficients for the rudder
    //and compute Clr/Cdr from the lookup table using interpolation.
    double xp ;
    double Clk;
...
void CSailboatMotionEquation::RestoreForce() {
...
void CSailboatMotionEquation::DampingForce() {
    ...
    Damping(par->rou,par->Ak,par->x_k,par->y_k,par->z_k,0);

    Damping(par->rou,par->Ah,par->x_h,par->y_h,par->z_h,1);
...
void CSailboatMotionEquation::Damping(double rou, double A, double x, double y, double z, int id) {
    ...
    if (id == 0)
    {
        D_keel_tmp[0] = tmp1;
        D_keel_tmp[1] = tmp2;
        D_keel_tmp[2] = -tmp2*z;  //why?
        D_keel_tmp[3] = tmp2*x;
    } else if (id == 1)
    {
        D_hull_tmp[0] = tmp1;
        D_hull_tmp[1] = tmp2;
        D_hull_tmp[2] = -tmp2*z;  //why?
        D_hull_tmp[3] = tmp2*x;
    }
    //cout<<"Dh_tmp"<<endl<<Dh_tmp[0]<<endl;
    //return Dh_tmp;
}
void CSailboatMotionEquation::ControlForceRudder() {
    ...
    while (fabs(alphaTmp)>pi)
    {
        if (alphaTmp>0)
            alphaTmp = alphaTmp - 2*pi;
        else
            alphaTmp = alphaTmp + 2*pi;
    }
    double V2 = pow(v_au,2) + pow(v_av,2);
...
void CSailboatMotionEquation::ControlForceSail() {
    MatrixXd J1(2,2);
    J1(0,0) = cos(psi);
    J1(0,1) = sin(psi);
    ...
    if(fabs(v_aw(1,0)) < 0.001 && fabs(v_aw(0,0)) < 0.001 )
        ...
        beta = atan2(-v_aw(1,0),-v_aw(0,0));

    double alphaTmp = delta_s + beta;
...
void CSailboatMotionEquation::Equation() {
...
double CSailboatMotionEquation::d2r(double d) {
    return d*pi/180;
}
