...
from dueling_dqn_brain import DuelingDQN
import numpy as np
import os
import datetime
...
def run_maze():
    ...
    for episode in range(8000):
        ...
        while True:
            ...
            if (step >  MEMORY_SIZE) and (step % 5 == 0):
                RL.learn()
    ...
    print('finish dueling DQN training')
...
def analyze():
    ...
    print('finish dueling DQN analyze')
...
if __name__ == "__main__":
    # maze game
    env = Maze()
    model_path = os.path.dirname(os.path.abspath(__file__)) + "/../model/dueling_dqn/dueling_dqn_model_20171222.ckpt"
    RL = DuelingDQN(env.n_actions, env.n_features, model_path,
                      learning_rate=0.01,
                      reward_decay=0.9,
                      e_greedy=0.9,
