//
// Created by xie hanqing on 10-04-19
//
#include "ros/ros.h"
...
static double end_x = 10;
static double end_y = 10;
static double object_x = 5;
static double object_y = 5;
static int count = 2;

static int obs_switch = 0;
// 0 get message from "/object/pose"
// 1 not object
// 2 have object

static int find_obs = 0;
...
void cfg_cb(path_planning_astar::path_planning_Config &config, uint32_t level) {
    ...
    if (count != config.count){
        ...
        for (int i = 0; i<count;i++){
            if (i%2==0){
                Eigen::Vector2d tar_1(start_x,start_y);
                target_points_ne.push_back(tar_1);
            }
            else{
                Eigen::Vector2d tar_1(end_x,end_y);
                target_points_ne.push_back(tar_1);
            }
        }
    ...
    object_x = (start_x+end_x)/2;
    object_y = (start_y+end_y)/2;
}


void obs_cb(const geometry_msgs::PoseArrayConstPtr &obs_in) {
    double dis = sqrt(pow(start_x-end_x,2)+pow(start_y-end_y,2))/2;
    int num = obs_in->poses.size();
    if (find_obs == 0 and (nums>0){
        for (int i;i<num;i++){
            double x = obs_in->poses[i].x;
            double y = (obs_in->poses[j].y;
            double r = sqrt(pow(x-object_x,2)+pow(y-object_y,2));
            if (find_obs == 0 and r<dis){
                Eigen::Vector2d obj_ne(object_x, object_y);
                obs_ne_vector_0.push_back(obj_ne);
                find_obs = 1;
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber obs_sub = nh.subscribe("/object/pose", 2, &obs_cb);
